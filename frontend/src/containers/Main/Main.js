...
const Main = (props) => {
  ...
  useEffect(() => {
    socket.on("addBet", (data) => {
      ...
      try {
        if (
          data.bet.user === JSON.parse(localStorage.getItem("userData")).userId
        ) {
    ...
    socket.on("getBets", (data) => {
      try {
        data.bets.forEach((bet) => {
          if (
            bet.user === JSON.parse(localStorage.getItem("userData")).userId
          ) {
            updateUserBet(bet);
          }
        });
    ...
    socket.on("timerFinish", (data) => {
      try {
        if (
          !userBet.won &&
          userBet.koef <=
            parseFloat(data.koef / 1000 + "." + (data.koef % 1000) / 100)
        ) {
  ...
  useEffect(() => {
    socket.once("changeBet", (data) => {
      ...
      addBet(
        bets.map((item, index) => (item._id === data.bet._id ? data.bet : item))
      );
    });
    // socket.once('changeBet',data => {
    //     addBet(
        ...
        <div className={classes.BetCards}>
          {bets.map((betInfo, index) =>
            betInfo.won === undefined ? (
              ...
              <>
                <BetCard betInfo={betInfo} key={index} status="Success" />
              </>
            ) : (
              <>
                <BetCard betInfo={betInfo} key={index} status="Failed" />
              </>
            )
          )}
        </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    setBalance: (value) =>
      dispatch({ type: actionTypes.SET_BALANCE, value: value }),
  };
};
