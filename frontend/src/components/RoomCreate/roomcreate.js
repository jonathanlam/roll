      ...
      {auth.isLoggedIn ? (
        <>
          ...
          <button
            ...
            onClick={() => props.maxAmountSetHandler(props.balance)}
          >
            max
          </button>
        </>
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateBetValue: (event) =>
      dispatch({ type: actionTypes.UPDATE_BET_VALUE, event: event }),
    clearAmountHandler: () => dispatch({ type: actionTypes.CLEAR_BET_VALUE }),
    maxAmountSetHandler: (maxValue) =>
      dispatch({ type: actionTypes.ADD_MAX_BET_VALUE, value: maxValue }),
  };
};
